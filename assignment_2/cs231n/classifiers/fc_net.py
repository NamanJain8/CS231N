...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss,ds=softmax_loss(scores,y) #ds=(N,num_classes) is gradient for scores
        loss+= 0.5*self.reg*(np.sum(np.square(self.params['W1']))+np.sum(np.square(self.params['W2'])))

        gradsa1,grads['W2'],grads['b2']=affine_backward(ds,cache2)
        _,grads['W1'],grads['b1']=affine_relu_backward(gradsa1,cache1)
        grads['W1']+=self.reg*self.params['W1']
        grads['W2']+=self.reg*self.params['W2']
        #bias terms are not regularised
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Initialise for layers (except last)
        ...
        # Initialise for last layer (not using Batch normalization for last layer)
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        output=X            # stores output and cache for further use in back-prop
        af_cache=[0]            # 0-indexing
        bn_cache=[0]
        relu_cache=[0]
        do_cache=[0]

        # Use Affine _forward only for last layer
        for i in range(self.num_layers):
            if i is not self.num_layers-1:
                output,cache=affine_forward(output,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
                af_cache.append(cache)
                if self.use_batchnorm:
                    output,cache=batchnorm_forward(output,self.params['gamma'+str(i+1)],self.params['beta'+str(i+1)],self.bn_params[i])
        ...
        dx_last,dw_last,db_last=affine_backward(dsoft,af_cache[self.num_layers])
        grads['W'+str(self.num_layers)]=dw_last+self.reg*self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)]=db_last
        #for having num_layers to 0 layers 
        for i in range(self.num_layers-1,0,-1):
            if self.use_dropout:
                dx_last=dropout_backward(dx_last,do_cache[i])
            dx_last=relu_backward(dx_last,relu_cache[i])
            if self.use_batchnorm:
                dx_last,dgamma,dbeta=batchnorm_backward(dx_last,bn_cache[i])
                grads['gamma'+str(i)]=dgamma
                grads['beta'+str(i)] =dbeta
            dx_last,dw_last,db_last=affine_backward(dx_last,af_cache[i])
